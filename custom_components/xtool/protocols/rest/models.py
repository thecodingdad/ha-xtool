...
from .protocol import RestProtocol, RestProtocolF1V1, RestProtocolM1Legacy

XTOOL_M1 = XtoolDeviceModel(
    model_id="M1",
    name="xTool M1",
    protocol_class=RestProtocolM1Legacy,
...
XTOOL_F1 = XtoolDeviceModel(
    model_id="F1",
    name="xTool F1",
    protocol_class=RestProtocolF1V1,
    coordinator_class=RestCoordinator,
    has_beeper=True,
    has_flame_alarm=True,
    has_move_stop=True,
    ...
    has_mode_switch=True,
    # F1 (basic / portable galvo) has no camera, no fill-light, no
    # gantry — it's a galvo head with laser-based preview path. Removed
    # has_camera_exposure / has_laser_head_position / has_fill_light
    # which were inherited from the F1 Ultra model definition.
...
XTOOL_F1_ULTRA = XtoolDeviceModel(
    ...
    has_cover_sensor=True,
    # F1 Ultra uses internal/passive cooling (galvo MOPA fiber + diode);
    # no water-cooling loop per xTool support docs. Water-cooling flag
    # is reserved for the CO2 line (P2 / P2S / P3).
